...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        paddedX = pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(paddedX)
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0]
        enc_hiddens = enc_hiddens.permute(1, 0, 2)


        #3 - Compute `dec_init_state`
        forwardTensorH = last_hidden[0]
        backwardTensorH = last_hidden[1]
        concatTensorH = torch.cat([forwardTensorH, backwardTensorH], dim = 1)
        init_decoder_hidden = self.h_projection(concatTensorH)

        forwardTensorC = last_cell[0]
        backwardTensorC = last_cell[1]
        concatTensorC = torch.cat([forwardTensorC, backwardTensorC], dim = 1)
        init_decoder_cell = self.c_projection(concatTensorC)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #1 - Obtain new dec_state
        dec_state_hidden, dec_state_cell = dec_state
        dec_state = self.decoder(Ybar_t, (dec_state_hidden, dec_state_cell))
        #2 - Split dec_state
        dec_state_hidden, dec_state_cell = dec_state
        #3 - Compute attention scores 
        e_t = enc_hiddens_proj.bmm(dec_state_hidden.unsqueeze(2)).squeeze(2)
        ### END YOUR CODE
        ...
        a_t = alpha_t.bmm(enc_hiddens)
        a_t = a_t.squeeze(1)

        #3 - U_t
        U_t = torch.cat([dec_state_hidden, a_t], 1)

        #4 - Combined Output
        V_t = self.combined_output_projection(U_t)

        #5
        nonLinear = torch.tanh(V_t)
        O_t = self.dropout(nonLinear)
